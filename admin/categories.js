(() => {
  ...
  async function workerWrite(payload){
    const response=await fetch(`${API}/api/file`,{method:'PUT',headers:{Accept:'application/json','Content-Type':'application/json',...(session()?{Authorization:`Bearer ${session()}`}:{})},body:JSON.stringify(payload)});
  ...
  function renderManager(){
    ...
    list.innerHTML=categories.map(category=>`<div class="category-row"><div><strong>${escapeHtml(category)}</strong><small>اسم القسم</small></div><button type="button" class="button button-danger" data-delete-category="${escapeHtml(category)}">حذف</button></div>`).join('');
  }
  ...
  function ensureUI(){
    ...
    admins.insertAdjacentHTML('afterend',`<section id="categories-view" class="view" hidden><div class="section-card"><div class="section-title-row"><div><p class="eyebrow">تنظيم المحتوى</p><h2>إدارة الأقسام</h2></div><span id="categories-count" class="role-badge">—</span></div><p class="muted">أضف أقساماً جديدة أو احذف قسماً غير مستخدم. التغييرات تحفظ في ملف واحد صغير، لذلك لا تؤثر على سرعة صفحات الزوار.</p><div class="admin-add-row"><label>اسم القسم<input id="new-category-input" type="text" maxlength="60" placeholder="مثال: جغرافيا" autocomplete="off"></label><button id="add-category-button" class="button button-primary" type="button">إضافة قسم</button></div><div id="categories-status" class="status" hidden></div><div id="categories-list" class="categories-list"></div></div></section>`);
    $('add-category-button')?.addEventListener('click',addCategory);$('new-category-input')?.addEventListener('keydown',event=>{if(event.key==='Enter')addCategory();});$('categories-list')?.addEventListener('click',event=>{const button=event.target.closest('[data-delete-category]');if(button)deleteCategory(button.dataset.deleteCategory);});
  }
  ...
  async function deleteCategory(category){
    if(categories.length<=1){showMessage('لا يمكن حذف آخر قسم.','error');return;}
    if(!window.confirm(`هل تريد حذف قسم «${category}»؟\n\nلن يتم حذف أي إنفوغرافيك، لكن لا يمكن اختيار هذا القسم لمنشورات جديدة بعد الحذف.`))return;
    try{await saveCategories(categories.filter(item=>item!==category));showMessage('تم حذف القسم بنجاح.','success');}catch(error){showMessage(error.message||'تعذر حذف القسم.','error');}
  }
