(() => {
  ...
  function decorateRows(){
    document.querySelectorAll('#posts-list .post-row').forEach(row=>{
      ...
      const edit=row.querySelector('[data-edit-index]'),remove=row.querySelector('[data-delete-index]'); if(!edit||!remove)return;
      const index=Number(edit.dataset.editIndex),post=window.__infografPosts?.[index]; if(!post)return;
      ...
      const button=document.createElement('button');button.type='button';button.className='button '+(marked?'button-primary':'button-secondary');button.dataset.featureIndex=index;button.textContent=marked?'إلغاء التمييز':'تمييز';remove.insertAdjacentElement('beforebegin',button);
    });
  }
  async function toggleFeatured(index,button){
    if(button)button.disabled=true;
    try{const post=window.__infografPosts?.[index];if(!post)throw new Error('تعذر العثور على الإنفوغرافيك.');const full=await window.InfografFast.getPost(post.path);let text=full.text||'';const enabled=!/^featured:\s*true\s*$/mi.test(text);const line='featured: '+(enabled?'true':'false');const updated=/^featured:\s*(?:true|false)\s*$/mi.test(text)?text.replace(/^featured:\s*(?:true|false)\s*$/mi,line):text.replace(/^---\s*\n/,'---\n'+line+'\n');
  ...
  document.addEventListener('click',event=>{const button=event.target.closest('[data-feature-index]');if(!button)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();toggleFeatured(Number(button.dataset.featureIndex),button);},true);
